...
export class ApiCallsService {
    ...
    async getPriceData(name: string, days: number): Promise<PriceData> {
        const dataPromise = await axios.get("https://api.coingecko.com/api/v3/coins/" + name + "/market_chart?vs_currency=usd&days=" + days + "&interval=daily");
    ...
    async getOtherData(name: string): Promise<OtherData> {
        const dataPromise = await axios.get("https://api.coingecko.com/api/v3/coins/" + name + "?localization=false&market_data=true");
